...
def update_product(id_producto: int, descripcion: str, precio: float, stock: int, id_rubro: int):
    ...
    Si el producto está asociado a una factura, solo se puede modificar el stock.
    """
    try:
        # Verificar si el producto está en alguna factura
        asociado = execute_query(
            "SELECT COUNT(*) FROM detalle_factura WHERE id_producto = ?", 
            (id_producto,), 
            fetch="one"
        )

        if asociado and asociado[0] > 0:
            # Solo permitir modificar stock
            execute_query("""
                UPDATE producto
                SET stock = ?
                WHERE id_producto = ?
            """, (stock, id_producto), commit=True)
            import tkinter.messagebox as messagebox
            messagebox.showinfo("Aviso", "Este producto está asociado a facturas, solo se modificó el stock.")
            print(f"[WARN] Producto {id_producto} asociado a facturas: solo se modificó stock.")
        else:
            # Si no está asociado, permitir modificar todo
            execute_query("""
                UPDATE producto
...
def delete_product(id_producto):
    """Elimina un producto solo si no está en ninguna factura."""
    detalles = execute_query(
        "SELECT COUNT(*) FROM detalle_factura WHERE id_producto = ?", 
        (id_producto,), 
        fetch="one"
    )
    if detalles and detalles[0] > 0:
        import tkinter.messagebox as messagebox
        messagebox.showerror("Error", "No se puede eliminar este producto porque está asociado a facturas.")
        return

    execute_query("DELETE FROM producto WHERE id_producto = ?", (id_producto,))
    print(f"[OK] Producto con ID {id_producto} eliminado correctamente.")
...
def update_client(id_cliente: int, nombre: str, id_provincia: int, domicilio: str, telefono: str, email: str):
    """
    Actualiza un cliente solo si no tiene facturas asociadas.
    """
    facturas = execute_query(
        "SELECT COUNT(*) FROM factura WHERE id_cliente = ?", 
        (id_cliente,), 
        fetch="one"
    )
    ...
    if facturas and facturas[0] > 0:
        import tkinter.messagebox as messagebox
        messagebox.showerror("Error", "No se puede modificar este cliente porque tiene facturas asociadas.")
        print(f"[WARN] Intento de modificar cliente {id_cliente} con facturas asociadas.")
        return
    ...
    print(f"[OK] Cliente {id_cliente} actualizado correctamente.")



def delete_client(id_cliente):
    """Elimina un cliente solo si no tiene facturas asociadas."""
    ...
    if facturas and facturas[0] > 0:
        import tkinter.messagebox as messagebox
        messagebox.showerror("Error", "No se puede eliminar este cliente porque tiene facturas asociadas.")
        return
    
    execute_query("DELETE FROM cliente WHERE id_cliente = ?", (id_cliente,))
    print(f"[OK] Cliente con ID {id_cliente} eliminado correctamente.")
...
def update_rubro(id_rubro: int, nombre_rubro: str):
    """
    Actualiza un rubro solo si ninguno de sus productos está asociado a una factura.
    """
    # Verificar si hay productos del rubro que estén en facturas
    asociado = execute_query("""
        SELECT COUNT(*) 
        FROM producto AS p
        JOIN detalle_factura AS df ON p.id_producto = df.id_producto
        WHERE p.id_rubro = ?
    """, (id_rubro,), fetch="one")
    ...
    if asociado and asociado[0] > 0:
        ...
        messagebox.showerror(
            "Error", 
            "No se puede modificar este rubro porque tiene productos asociados a facturas."
        )
        print(f"[WARN] Intento de modificar rubro {id_rubro} con productos en facturas.")
        return

    # Si no está asociado, permitir actualización
    execute_query(
        "UPDATE rubro SET nombre_rubro = ? WHERE id_rubro = ?",
        (nombre_rubro, id_rubro),
        commit=True
    )
    print(f"[OK] Rubro {id_rubro} actualizado correctamente.")


def delete_rubro(id_rubro: int):
    """
    Elimina un rubro solo si ninguno de sus productos está asociado a una factura.
    También evita eliminar si aún tiene productos en la tabla producto.
    """
    # 1️⃣ Verificar si tiene productos asociados a facturas
    asociado_facturas = execute_query("""
        SELECT COUNT(*) 
        FROM producto AS p
        JOIN detalle_factura AS df ON p.id_producto = df.id_producto
        WHERE p.id_rubro = ?
    ...
    if asociado_facturas and asociado_facturas[0] > 0:
        import tkinter.messagebox as messagebox
        messagebox.showerror(
            "Error",
            "No se puede eliminar este rubro porque tiene productos asociados a facturas."
        )
        print(f"[WARN] Intento de eliminar rubro {id_rubro} con productos en facturas.")
        return

    # 2️⃣ Verificar si tiene productos aunque no estén facturados
    productos = execute_query(
        "SELECT COUNT(*) FROM producto WHERE id_rubro = ?", 
        (id_rubro,), fetch="one"
    )
    if productos and productos[0] > 0:
        import tkinter.messagebox as messagebox
        messagebox.showerror(
            "Error",
            "No se puede eliminar este rubro porque aún tiene productos asociados."
        )
        print(f"[WARN] Intento de eliminar rubro {id_rubro} con productos asociados.")
        return

    # 3️⃣ Si no tiene productos, permitir eliminar
    execute_query(
        "DELETE FROM rubro WHERE id_rubro = ?", 
        (id_rubro,), 
        commit=True
    )
    print(f"[OK] Rubro {id_rubro} eliminado correctamente.")
